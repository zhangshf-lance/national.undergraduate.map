...
function chooseCoordinate(school, candidates) {
...
const targets = data.schools.filter((school) =>
  school.coordinateConfidence === "low"
  || String(school.coordinateSource || "").startsWith("manual")
  || /国防科技大学|海军军医大学|空军军医大学/.test(school.name)
);
...
for (const school of targets) {
  ...
  const selected = chooseCoordinate(school, Object.values(providerResults).flat());
  const before = {
    point: school.point,
    confidence: school.coordinateConfidence,
    source: school.coordinateSource,
  };
  if (selected) {
  ...
  cache.schools[school.id] = {
    ...
    selected: selected || {
      ...
      reason: "no reliable recalibration match",
    },
  ...
  changes.push({
    ...
    after: {
      ...
      confidence: school.coordinateConfidence,
      source: school.coordinateSource,
      candidate: school.coordinateCandidate?.name || "",
    },
  });
}
...
data.meta.coordinateCalibration = {
  ...
  note: "高/中置信坐标来自地图服务检索；低置信坐标保留省级聚合散点；新增军校坐标已重新用高德、天地图校准。",
};
...
console.log(JSON.stringify({ targets: targets.length, changes, counts: data.meta.coordinateCalibration.counts }, null, 2));
